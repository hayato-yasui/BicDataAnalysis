...
class NormalDistribution:
    ...
    def _test_normality(self, df_grouped, amount_col='販売数'):
        ...
        for key, df in df_grouped:
            ...
            # 歪度 : グラフがどれくらい平均の回りに対称に分布していないかを示す数値で、0に近ければ近いほど正規分布に近い数値
            df['歪度'] = pd.Series(values_li).skew()
            # 尖度 : グラフがどれくらい尖っているかを表す数値です。こちらは-3して0になる値が正規分布に近い数値
            df['尖度'] = pd.Series(values_li).kurtosis()
